...
class LatexCompiler:
    ...
    def remove_conflicting_packages(self, tex_file: Path) -> None:
        ...
        conflict_patterns = [
            ...
            # breakurl은 dvips/pdfTeX 전용 — XeLaTeX에서는 shipout 훅의
            # \headerps@out 미정의 에러 발생. hyperref가 URL 줄바꿈을 자체 처리.
            re.compile(r'\\(?:usepackage|RequirePackage)(\[.*?\])?\{breakurl\}'),
        ]
    ...
    def _relocate_font_packages(self, directory: Path) -> list[str]:
        ...
        매크로 본문·조건부 인자 등 중괄호 내부(깊이>0)의 로드는 건드리지 않는다.
        예: IEEEtran.cls의 comsoc 옵션 전용 fallback
        `\\def\\@IEEEcomsocenforcemathfont{...\\RequirePackage{newtxmath}}`을
        재배치하면 원본에 없던 newtxmath가 무조건 로드되어 amsthm(\\openbox)·
        amssymb(\\Bbbk)와 "already defined" 충돌을 일으킨다.
        ...
        for ext in ("*.cls", "*.sty"):
            for aux_file in directory.rglob(ext):
                ...
                try:
                    ...
                    modified = False
                    new_lines = []
                    depth = 0  # 현재 라인 시작 시점의 중괄호 깊이
                    for line in lines:
                        code = re.sub(r'(?<!\\)%.*', '', line)  # 주석 제외
                        m = pattern.search(code)
                        if m and depth + brace_delta(code[:m.start()]) == 0:
                            pkg_names = {
                            ...
                            if pkg_names & self.FONT_PACKAGES_TO_RELOCATE:
                                ...
                                if use_line not in relocated_lines:
    ...
    SPOTCOLOR_SHIM = r"""% XeLaTeX fallback shim for spotcolor.sty (generated by arXiv-translator)
% 원본 spotcolor.sty는 pdfTeX 프리미티브(\pdfobj, \pdfliteral 등)와
% [pdftex]{graphics} 드라이버를 강제하여 XeLaTeX에서 컴파일되지 않는다.
% 별색(spot color)을 \AddSpotColor에 지정된 CMYK 근사값으로 대체한다.
\NeedsTeXFormat{LaTeX2e}
\ProvidesPackage{spotcolor}[2006/11/15 v1.2 XeLaTeX fallback shim]
\RequirePackage{graphics}
\RequirePackage{color}
\providecommand{\SpotSpace}{ }
\newcommand{\NewSpotColorSpace}[1]{}
\newcommand{\SetPageColorSpace}[1]{}
\newcommand{\SetPageColorResource}[1]{}
\newcommand{\SpotColor}[2]{\color{spotcolor@#1}}
% \AddSpotColor{space}{name}{pdfname}{c m y k} → spotcolor@name 을 cmyk 로 등록
\newcommand{\AddSpotColor}[4]{\spotcolor@addcmyk{#2}#4 \@nil}
\def\spotcolor@addcmyk#1#2 #3 #4 #5 \@nil{\definecolor{spotcolor@#1}{cmyk}{#2,#3,#4,#5}}
% \definecolor{x}{spotcolor}{name,tint} → 등록된 cmyk 재사용 (tint 무시)
\def\color@spotcolor#1#2{\spotcolor@parse#2,\@nil#1}
\def\spotcolor@parse#1,#2\@nil#3{\expandafter\let\expandafter#3\csname\string\color@spotcolor@#1\endcsname}
\endinput
"""

    def _shim_spotcolor(self, directory: Path) -> None:
        """pdfTeX 전용 spotcolor.sty를 XeLaTeX용 대체 구현으로 교체

        IEEE Access 템플릿(ieeeaccess.cls)이 동봉하는 spotcolor.sty는
        \\pdfobj 등 pdfTeX 프리미티브를 쓰고 [pdftex]{graphics}를 강제한다.
        XeLaTeX에서는 프리미티브 미정의로 "Missing \\begin{document}"가 나고,
        pdftex.def 드라이버 때문에 모든 \\color가 \\pdfcolorstack 에러,
        모든 그림이 "not found"로 번진다. 원본은 spotcolor.sty_original로 보존.
        """
        for sty in directory.rglob("spotcolor.sty"):
            text = sty.read_text(encoding='utf-8', errors='replace')
            if '\\pdfobj' not in text:
                continue
            backup = sty.with_name('spotcolor.sty_original')
            if not backup.exists():
                backup.write_text(text, encoding='utf-8')
            sty.write_text(self.SPOTCOLOR_SHIM, encoding='utf-8')
            logger.info(f"  spotcolor.sty → XeLaTeX shim 교체: {sty.name}")
    ...
    def sanitize_translated_tex(self, tex_file: Path) -> None:
        ...
        for i, line in enumerate(translated_lines):
            ...
            # === 3b. \textbf{\cmd ...} → \textbf{{}\cmd ...} ===
            # 일부 cls(ieeeaccess 등)는 \def\textbf#1{{\bf #1}}, \long\def\bf#1{...}
            # 처럼 인자를 받는 \bf를 쓴다. 번역 어순 변경으로 매크로가 인자 선두에
            # 오면(\textbf{\secref{x} 및 ...}) \bf가 \secref만 삼켜 {x}가 고아가
            # 되고 caption/tabular 전체가 무너진다. 표준 LaTeX에서 {}는 무해.
            new_line = re.sub(
                r'(\\(?:textbf|textit|emph|textsc|texttt|textrm|textsf)\{)'
                r'(\\[a-zA-Z]+)',
                r'\1{}\2', fixed_line
            )
            if new_line != fixed_line:
    ...
    def compile_directory(
        ...
        # pdfTeX 전용 spotcolor.sty (IEEE Access 템플릿) → XeLaTeX shim
        self._shim_spotcolor(directory)
